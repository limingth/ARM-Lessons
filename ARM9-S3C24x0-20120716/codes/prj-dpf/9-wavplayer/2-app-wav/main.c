...
void playwav(int wav_addr)
{
	int counter = 0;
	char vol = 0; 

	short * p = (short *)wav_addr;
	int size = 882046;
	int flag;
	...
	if (flag == 0x10)
	{
		...
		{	
			p = (short *)(wav_addr + 0x2c);
			size = *(int *)(wav_addr + 0x28);
		}
		else // "fact"
		{
			p = (short *)(wav_addr + 0x2c + 12);
			size = *(int *)(wav_addr + 0x28 + 12);		
		}
	}
	else if (flag == 0x12)
	{
